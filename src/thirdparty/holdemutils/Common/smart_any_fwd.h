...
struct close_delete
{
	...
	static void close( T * p )
	{
		...
		static detail::_static_assert<0 != sizeof( T )> const cannot_delete_an_incomplete_type;

#ifdef SMART_ANY_PTS
		// This checks to make sure we're not calling delete on a HANDLE
		static detail::_static_assert<!detail::is_handle<T*>::value> const cannot_delete_a_handle;
#endif
...
typedef value_const<UINT, UINT(-1)>                          invalid_handle_t;
//#define invalid_handle_t (HANDLE)(-1)
#endif
