...
FOR /D %%t IN (%TS_DIR%/%LANG%) DO lupdate %LOPTIONS% %UTILS_DIR%/utils.pro -ts %TS_DIR%/%%t/ramblercontactsutils.ts
FOR /D %%t IN (%TS_DIR%/%LANG%) DO lupdate %LOPTIONS% %LOADER_DIR%/loader.pro -ts %TS_DIR%/%%t/ramblercontacts.ts
