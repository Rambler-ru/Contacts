		...
		if (plugin)
		{
			...
			if (FRegistration)
			{
				...
				connect(FRegistration->instance(),SIGNAL(registerError(const QString &, const QString &, const QString &)),
					SLOT(onRegisterErrorReceived(const QString &, const QString &, const QString &)));
			}
		}
	}
...
bool MultiUserChatPlugin::initObjects()
{
	...
	if (FNotifications)
	{
		...
		inviteType.order = OWO_NOTIFICATIONS_MUC_INVITE_MESSAGE;
		inviteType.title = tr("When receiving an invitation to the conference");
		inviteType.kindMask = INotification::RosterNotify|INotification::TrayNotify|INotification::PopupWindow|INotification::SoundPlay|INotification::AutoActivate;
		...
		privateType.order = OWO_NOTIFICATIONS_MUC_PRIVATE_MESSAGE;
		privateType.title = tr("When receiving a new private message in conference");
		privateType.kindMask = INotification::TrayNotify|INotification::PopupWindow|INotification::SoundPlay|INotification::AlertWidget|INotification::TabPageNotify|INotification::ShowMinimized|INotification::AutoActivate;
		...
		groupchatType.order = OWO_NOTIFICATIONS_MUC_GROUPCHAT_MESSAGE;
		...
		mentionType.order = OWO_NOTIFICATIONS_MUC_MENTION_MESSAGE;
...
INotification MultiUserChatPlugin::notifyMessage(INotifications *ANotifications, const Message &AMessage)
{
	...
	if (!multiChatWindow(AMessage.to(),roomJid))
	{
		...
		if (notify.kinds > 0)
		{
			...
			notify.data.insert(NDR_ROSTER_FLAGS,IRostersNotify::Blink|IRostersNotify::AllwaysVisible);
			...
			notify.data.insert(NDR_POPUP_IMAGE,ANotifications->contactAvatar(AMessage.to(),fromJid));
			notify.data.insert(NDR_POPUP_TEXT,Qt::escape(notify.data.value(NDR_POPUP_TEXT).toString()));
...
void MultiUserChatPlugin::onRosterIndexContextMenu(IRosterIndex *AIndex, QList<IRosterIndex *> ASelected, Menu *AMenu)
{
...
void MultiUserChatPlugin::onRegisterErrorReceived(const QString &AId, const QString &AError, const QString & AMessage)
{
	Q_UNUSED(AError);
	Q_UNUSED(AMessage);
	if (FNickRequests.contains(AId))
